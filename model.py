...
if usePickles:
    ...
    augmented_measurements = np.load('./numpy_pickles/augmented_measurements.npy')
    augmented_images = np.load('./numpy_pickles/augmented_images.npy')
    print("Success")
...
if myData:
    ...
    # Load Pickled Data
    if usePickles:
        myImages = np.load('./numpy_pickles/my_images.npy')
        myMeasurements = np.load('./numpy_pickles/my_measurements.npy')
    ...
    # Load Data, save pickles
        ...
        np.save('./numpy_pickles/my_images.npy', myImages)
        np.save('./numpy_pickles/my_measurements.npy', myMeasurements)
    ...
    HF.print_histogram(augmented_measurements, show=show, title="Combined Data, after data balance")

    # Save Augmented Array Pickles
    np.save('./numpy_pickles/augmented_measurements', augmented_measurements)
    np.save('./numpy_pickles/augmented_images', augmented_images)
...
'''---------------------------------------
        CheckPoint
---------------------------------------'''
checkpoint = ModelCheckpoint('model-challenge-Warped-{epoch:02d}.h5',
                             monitor='val_loss',
                             verbose=2,
...
'''---------------------------------------
        Save and Exit
---------------------------------------'''
model.save('model-challenge-warped.h5')
print("Model Saved")
exit()
